...
def rsc(sys, controller, lqr, data_dict, eta=np.radians(15), limit=0.32, V_max=4):

    # walking
    step_lim = np.cos(np.radians(90)-eta)*sys.Lp * 3
    step = np.linspace(-step_lim, step_lim, 7)
    init = 1
    runner = 0
    decider = True
    ...
    try:
        ...
        for k_loop in range(n):
            ...
            if -eta < ((state[1, k_loop] + np.pi) % (2 * np.pi) - np.pi) < eta and lqr is not None:
                if switch == 0:
                    ...
                    switch = 1
                if runner > 5/dt:
                    lqr.set_goal(np.array([step[init], 0, 0, 0]))
                    if (state[0, k_loop] - 0.015 * (-1)**switch >= step[init]) == decider:
                        print("goal")
                        if init == len(step)-1 or switch == 2:
                            init -= 1
                            switch = 2
                            decider = False
                        if init == -1 or switch == 1:
                            if init == -1:
                                init = 0
                            init += 1
                            switch = 1
        ...
        print(f'Average control frequency: {(k_loop-1) / time_end} Hz')
