...
class DrakeStepSimulator():
    def __init__(self, urdf_path, data_dict, force_limit, dt_sim = 0.01):
        ...
        R_tilqr = np.array([1])
        tilqr_context.SetContinuousState(self.xG)
    ...
    def simulate(self, x0 = None, init_knot = 0, final_knot = -1):
        ...
        if x0 is None:
            x0 = self.x0
        # context.SetContinuousState(self.xG)
        context.SetTime(t0)
        ...
        print(f'timestep: {tf/(len(x_sim.T)+1)}')
...
class StepSimulator():
    ...
    def simulate(self, sys, x0 = None, init_knot = 0, final_knot = -1, integrator = "euler"):
        ...
        # Simulate
        h = self.dt_sim
        N = int(len(self.T_nom)/self.dt_sim)
        x_sim = np.zeros((N,4))
        x_sim[0] = x0
        u_sim = np.zeros((N,1))
        t_sim = np.zeros((N,1))
        for i in range(N-1):
                x_star = self.controller.x0.value(t_sim[i]).T[0]
                u_star = self.controller.u0.value(t_sim[i])[0][0]
                u_sim[i+1] = - self.controller.K.value(t_sim[i]).dot((x_sim[i]-x_star))

                # Input saturation
                if u_sim[i+1]<= -self.force_limit:
                    u_sim[i+1] = -self.force_limit
                elif u_sim[i+1]>= self.force_limit:
                    u_sim[i+1] = self.force_limit 

                x_sim[i+1] = self.integration(integrator,sys,h,x_sim[i],u_sim[i+1], t_sim[i])

                # State saturation warning
                if x_sim[i+1][0]<= -0.3 or x_sim[i+1][0]>= 0.3:
    ...
    def integration(self,type, sys, h, x_i, u_iplus1, t_i):
        ...
        if type == "rk4":
            ...
            x_star = self.controller.x0.value(t_i + h/2).T[0]
            u_iplus1 = - self.controller.K.value(t_i + h/2).dot((x_i + K1/2-x_star))
            K2 = h*sys.continuous_dynamics_RoA(x_i + K1/2, u_iplus1)
            x_star = self.controller.x0.value(t_i + h/2).T[0]
            u_iplus1 = - self.controller.K.value(t_i + h/2).dot((x_i + K2/2-x_star))
            K3 = h*sys.continuous_dynamics_RoA(x_i + K2/2, u_iplus1)
            x_star = self.controller.x0.value(t_i + h).T[0]
            u_iplus1 = - self.controller.K.value(t_i + h/2).dot((x_i + K3-x_star))
