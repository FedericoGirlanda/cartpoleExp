...
from trajectory_optimization.ilqr.pendulum import discrete_dynamics_rungekutta

def real_system_simulation(sys, controller, lqr, data_dict, eta=np.radians(10), limit=0.35, V_max=6, disturbance = False):
    ...
    try:
        ...
        for k_loop in range(n):
            ...
            if -eta < ((state[1, k_loop] + np.pi) % (2 * np.pi) - np.pi) < eta and lqr is not None:
                ...
                print(f'1: {k_loop, time.time()- time_0}')
